...
BEGIN
	...
	set @varlist := (select group_concat(distinct variable) from powell where region='RR' and site='SS');
	update site set variableList=@varlist where region='RR' and site='SS';
	set @firstrec := (select min(DateTime_UTC) from powell where region='RR' and site='SS');
	update site set firstRecord=@firstrec where region='RR' and site='SS';
	set @lastrec := (select max(DateTime_UTC) from powell where region='RR' and site='SS');
